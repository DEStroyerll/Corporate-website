<!--Шаблон для работы со слайдерами-->
@if(count($sliders) > 0)
    <div id="slider-cycle" class="slider cycle no-responsive slider_cycle group" style="height:485px;">
        <ul class="slider">

            <!--Здесь мы создали собственную директиву-->
            @setDirective($index, 1)

            <!--Циклом проходимся по коллекции $sliders-->
            @foreach($sliders as $slider)
                <li>
                    <!--Здесь мы указали относительное имя папки для изображения-->
                    <div class="slide-holder" style="background:  url('{{asset(env('THEME'))}}/images/{{$slider->img}}') no-repeat center center" style="height:483px;">
                        <div class="slide-content-holder inner" style="height:483px;">
                            ...
                            @else
                                <div class="slide-content-holder-content" style="position: absolute; top:80px;left:500px;"></div>
                            @endif
