...
<div id="content-blog" class="content group">
        ...
        @foreach($articles as $article)
            <div class="sticky hentry hentry-post blog-big group">
                ...
                <div class="meta group">
                    ...
                    <p class="comments"><span><a href="{{route('articles.show', ['alias'=>$article->alias])}}#respond" title="Comment on Section shortcodes &amp; sticky posts!">{{count($article->comment) ? count($article->comment) : '0'}}{{Lang::choice('', count($article->comment))}}</a></span></p>
                </div>
                <!-- post content -->
                <div class="the-content group">
                   {!! $article->description !!}
                    <p><a href="{{route('articles.show', ['alias'=>$article->alias])}}" class="btn   btn-beetle-bus-goes-jamba-juice-4 btn-more-link">→ {{Lang::get('my_translations.read_more')}}</a></p>
                </div>
                <div class="clear"></div>
            </div>
        @endforeach
    ...
    <div class="general-pagination group">
        <!--Метод lastPage() возвращает номер последней страницы постраничной навигации-->
